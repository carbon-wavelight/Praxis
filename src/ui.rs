...
use druid::{Widget, WidgetExt, Env, EventCtx, Point, Menu, MenuItem, UnitPoint, Event, Code, Color};
use druid::widget::{Label, Flex, TextBox, Button, List, Checkbox, ZStack, Padding, Controller};

use crate::data::{TodoState, TodoItem};
use crate::saver::Saver;

// Returns anything that implements a widget
    ...
    let header = Flex::row()
        ...
        .with_child(
            ...
            .on_click(|_ctx, data: &mut TodoState, _env| {
                // Trim whitespace and check if empty/null
...
impl <W:Widget<TodoState>> Controller<TodoState, W> for Enter {
    // Function to use Enter key to push item to the list.
